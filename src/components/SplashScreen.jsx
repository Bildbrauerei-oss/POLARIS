...
export default function SplashScreen({ onDone }) {
  ...
  const progress = Math.min(tick / 100, 1)

  const dots = Array.from({ length: 1200 }, (_, i) => ({
    lat: (Math.random() - 0.5) * 180,
    lng: (Math.random() - 0.5) * 360,
    size: Math.random() * 0.5 + 0.15,
    color: i % 40 === 0 ? '#ffa600' : i % 15 === 0 ? '#ffffff' : i % 8 === 0 ? '#a7d5dc' : '#52b7c1',
  }))
  ...
  return (
    ...
    >
      ...
      <div style={{ position: 'absolute', inset: 0, background: 'radial-gradient(ellipse 120% 80% at 50% 60%, rgba(82,183,193,0.06) 0%, rgba(45,60,75,0.08) 40%, transparent 70%)', pointerEvents: 'none' }} />
      ...
      <div style={{ position: 'absolute', inset: 0, display: 'flex', alignItems: 'center', justifyContent: 'center', pointerEvents: 'none' }}>
        {GlobeComponent ? (
          <motion.div initial={{ opacity: 0, scale: 0.85 }} animate={{ opacity: 0.85, scale: 1 }} transition={{ duration: 2, ease: 'easeOut' }}>
            <GlobeComponent
              ref={globeRef}
              ...
              arcColor="color" arcAltitude={0.3} arcStroke={0.4}
              arcDashLength={0.4} arcDashGap={0.6} arcDashAnimateTime={3000}
      ...
      {/* Edge darkening */}
      <div style={{ position: 'absolute', inset: 0, background: 'radial-gradient(ellipse 50% 50% at 50% 50%, transparent 20%, rgba(2,8,16,0.7) 70%, rgba(2,8,16,0.95) 100%)', pointerEvents: 'none' }} />
      ...
      <div style={{ position: 'relative', zIndex: 10, textAlign: 'center' }}>
        ...
        <motion.h1 initial={{ opacity: 0, scale: 0.8, y: 30 }} animate={{ opacity: 1, scale: 1, y: 0 }} transition={{ delay: 0.4, duration: 1.2, ease: [0.16, 1, 0.3, 1] }}
        ...
        <motion.div initial={{ scaleX: 0, opacity: 0 }} animate={{ scaleX: 1, opacity: 1 }} transition={{ delay: 1.2, duration: 0.8 }}
        ...
        {/* Progress bar */}
        <motion.div initial={{ opacity: 0 }} animate={{ opacity: 1 }} transition={{ delay: 3 }} style={{ marginTop: '2.5rem' }}>
      ...
      {/* Scan line */}
      <motion.div animate={{ y: ['-100vh', '100vh'] }} transition={{ duration: 4, repeat: Infinity, ease: 'linear', delay: 2 }}
  ...
  return (
    <div style={{ position: 'relative', width: 700, height: 700, display: 'flex', alignItems: 'center', justifyContent: 'center' }}>
      {[700, 500, 350, 200].map((size, i) => (
        <motion.div key={size} animate={{ rotate: i % 2 === 0 ? 360 : -360, opacity: [0.03, 0.08, 0.03] }} transition={{ duration: 20 + i * 8, repeat: Infinity, ease: 'linear' }}
          style={{ position: 'absolute', width: size, height: size, border: `1px solid rgba(82,183,193,${0.15 - i * 0.03})`, borderRadius: '50%' }} />
      ))}
    </div>
  )
}
